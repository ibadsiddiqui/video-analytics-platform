    ...
    >
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          {isLimitReached && <AlertCircle className="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" />}
          {isWarning && <AlertCircle className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" />}
          {!isLimitReached && !isWarning && <Lock className="w-5 h-5 text-slate-600 flex-shrink-0 mt-0.5" />}
        ...
        <div className="w-full sm:w-40 flex-shrink-0">
          <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${Math.max(0, (requestsRemaining / requestsLimit) * 100)}%` }}
